...
class App extends Component {
  ...
  rQuote() {
    ...
    this.setState({
      quote: allQuotes[random],
      author: "-" + allAuthors[random]
    });
  ...
  button(ev) {
    ...
    document
      .querySelector("#text")
      .animate([{ opacity: 1 }, { opacity: 0 }], { duration: 500 }); //fade out
    document
    ...
    setTimeout(
      () =>
        (document.querySelector("#tweet-quote").href =
          ...
          encodeURIComponent('"' + this.rQuote() + '" -' + this.state.author)),
      400
    ); //change quote and twitter link update
    setTimeout(function () {
      document
        .querySelector("#text")
  ...
  render() {
    return (
      <div id="background" style={{ backgroundColor: this.state.color }}>
        <div id="quote-box">
          <h1 id="text" style={{ color: this.state.color }}>
            {this.state.quote}
          </h1>
        ...
        <div className='footer'>
              <p>
                ...
                {' @ 2022. '}
                <a href='https://github.com/xaleel/random-quote' target="_blank" rel="noopener noreferrer">Source code</a>.
              </p>
          </div>
      </div>
    );
